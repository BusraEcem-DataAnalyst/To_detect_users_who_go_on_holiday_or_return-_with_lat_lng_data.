...
with dailyavg as (
select user_id, timestamp_date,avg(lat) as lat,avg(lng) as lng from analytics.fmr
...
, general_location as (
select user_id,avg(lat) as home_lat,avg(lng) as home_lng from analytics.fmr
where timestamp_date>='2021-01-01'
and lat is not null
and lng is not null
and lat != 0
...
lat,
lng,
lag(lat, 1) over (partition by user_id order by user_id,timestamp_date) as lag_lat, /* Get the previous lat average from the daily calculated averages with the lag() function. */
lag(lng, 1) over (partition by user_id order by user_id,timestamp_date) as lag_lng, /* Get the previous lng average from the daily calculated averages with the lag() function. */
